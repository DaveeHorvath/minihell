...
/*   Updated: 2024/04/05 16:32:09 by ivalimak         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_rl_hist_load(const char *fname)
{
	...
	fd = open(ft_strsjoin(msh_getenv("HOME"), fname, '/'), O_RDONLY);
	if (fd < 0)
		return ;
...
void	ft_rl_hist_save(const char *fname)
{
	...
	fd = open(ft_strsjoin(msh_getenv("HOME"), fname, '/'), O_WRONLY | O_CREAT,
			0644);
	if (fd < 0)
		return ;
